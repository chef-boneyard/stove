...
module Stove::Error
  ...
  describe JiraConfigurationValidationFailed do
    it 'raises an exception with the correct message' do
      expect { raise described_class }.to raise_error { |error|
        expect(error).to be_a(described_class)
        expect(error.message).to eq <<-EOH.gsub(/^ {10}/, '')
          The Stove configuration for JIRA does not exist! In order to use the JIRA plugin, you must set your JIRA information in the `~/.stove' file:
  ...
  describe JiraPasswordValidationFailed do
    it 'raises an exception with the correct message' do
      expect { raise described_class }.to raise_error { |error|
        expect(error).to be_a(described_class)
        expect(error.message).to eq <<-EOH.gsub(/^ {10}/, '')
          The Stove configuration for JIRA does not contain a password. In order to use JIRA features, you must set a password in the jira section of the `~/.stove' file:
  ...
  describe JiraUsernameValidationFailed do
    it 'raises an exception with the correct message' do
      expect { raise described_class }.to raise_error { |error|
        expect(error).to be_a(described_class)
        expect(error.message).to eq <<-EOH.gsub(/^ {10}/, '')
          The Stove configuration for JIRA does not contain a username. In order to use JIRA features, you must set a username in the jira section of the `~/.stove' file:
